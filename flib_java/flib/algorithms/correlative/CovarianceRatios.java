...
public class CovarianceRatios{
	...
	public CovarianceRatios(final double[][] set, final double[] labels, int np, int type, double tol){
		...
		if (type>0){
			...
			if (type<=2&&type>=-2){
				for (int i=0; i<nclass; i++){
					...
					DenseDoubleMatrix2D covZ = new DenseDoubleMatrix2D(cv);
					DenseDoubleSingularValueDecomposition svd = new DenseDoubleSingularValueDecomposition(new DenseDoubleMatrix2D(cv_i),true,true);
					...
					DenseDoubleEigenvalueDecomposition eig = new DenseDoubleEigenvalueDecomposition(alg.mult(icov,covZ));
				...
				for (int i=0; i<nclass; i++){
					...
					for (int k=0; k<dim; k++){
						for (int l=0; l<dim; l++){
							cv_i[k][l] = m2[i][k][l]/np+(cvt[k][l]-m2[i][k][l])/(nclass-1)/np-2*m[i][k]/np*(mt[l]-m[i][l])/(nclass-1)/np;
						}
					}
					DenseDoubleMatrix2D covZ = new DenseDoubleMatrix2D(cv_i);
					//System.out.println("created matrix");
					DenseDoubleEigenvalueDecomposition eig = new DenseDoubleEigenvalueDecomposition(covZ);
					this.eigv[i] = extractEigenvectors(eig);
					this.eigs[i] = extractEigenvalues(eig);
		...
		// else we do normal pca on the set
			...
			eigv = new double[1][][];
			eigs = new double[1][];
			DenseDoubleEigenvalueDecomposition eig = new DenseDoubleEigenvalueDecomposition(new DenseDoubleMatrix2D(CovarianceMatrix.covariance(set_tot)));
	...
	private double[][] extractEigenvectors(DenseDoubleEigenvalueDecomposition eig){
		int n = eig.getV().rows();
	...
	private double[] extractEigenvalues(DenseDoubleEigenvalueDecomposition eig){
		double[] eigt = eig.getRealEigenvalues().toArray();
