...
public class PermClustering {
	...
	public static double ordering(final double[][] trainingset, int[][] orderset, final boolean[] categorical, final double[] weights, final double[] sigma, int scrit, double scrdim, int maxit, double comp, double err){
	...
	private static int update(final double[] weights, final boolean[] categorical, int[][] orderset, final double[][] neighborlist, final int[][] upper, final int[][] lower, final int[][][] dim_count, final int[][][] dim_count2, final ArrayList<ArrayList<TreeSet<SortPair>>> dm_out, final double[] sigma, double err){
		final int n = orderset[0].length;
		int nclust = orderset.length;
		final int d = neighborlist.length;
		...
		for (int i=0; i<rp.length; i++){
			...
			//if (x!=y&&((!u&(x!=lower[z][y]))||(u&(x!=upper[z][y])))&&((lower[z][y]==-1||upper[z][y]==-1))&&((lower[z][x]==-1||upper[z][x]==-1))&&y!=lower[z][x]&&y!=upper[z][x]){
				...
				final ArrayList<ArrayList<Integer>> points = new ArrayList<ArrayList<Integer>>();
				for (int j=0; j<d; j++){
					points.add(new ArrayList<Integer>());
				}
				final boolean[] complete = new boolean[NUM_CORES];
				final Future<?>[] fut = new Future<?>[NUM_CORES];
				try {
					for (int j=0; j<NUM_CORES; j++){
						final int j2 = j;
						fut[j2] = exec.submit(new Runnable() {
							@Override
							public void run(){
								try {
									for (int k=j2; k<d; k+=NUM_CORES){
										int lx = lower[z][x];
										int ux = upper[z][x];
										// all losses
										// losses of neighborhood x
										for (int l=0; l<sigma[0]; l++){
											double a = neighborlist[k][x];
											if (!categorical[k]){
												if (lx>-1){
													if (Math.abs(a-neighborlist[k][lx])<=sigma[1]){
														dim_count[z][k][x]--;
														points.get(k).add(k*n+x);
														dim_count[z][k][lx]--;
														points.get(k).add(k*n+lx);
													}
												}
												if (ux>-1){
											...
											if (uy>-1&&ly!=x&&uy!=x){
												if (!categorical[k]){
													if (Math.abs(neighborlist[k][ly]-neighborlist[k][uy])<=sigma[1]){
														dim_count[z][k][ly]--;
														points.get(k).add(k*n+ly);
														dim_count[z][k][uy]--;
														points.get(k).add(k*n+uy);
													}
												}
												else {
									...
									System.out.println("problem in swapping of the dimension reordering");
									t.printStackTrace();
								}
							}
						});
