...
public class LogGaborBand{
	...
	public LogGaborBand(final int[] w, double lambda1, double lambda2, double sigma, double phi, final double[] ang, double ang_width) {
		this.w = w.clone();
	...
	private void generate() {
		...
		t[0] = Math.PI/(Math.sqrt(2)*Math.log(2)*this.sigma);
	...
	private void nestedLoop(int l, double[] x, double[] t, int[] a, final int[] wcm, final double[] mw, final double[] wf){
		...
		if (l<1){
			for (int i=0; i<this.w[0]; i++){
				...
				t[1] = Erf.erf(Math.log(t[1]*lambda2)*t[0])-Erf.erf(Math.log(t[1]*lambda1)*t[0]);
